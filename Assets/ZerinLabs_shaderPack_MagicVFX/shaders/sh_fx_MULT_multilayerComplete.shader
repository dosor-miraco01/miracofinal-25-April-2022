Shader "zerinlabs/sh_fx_MULT_multilayerComplete"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				vertexOutput vert(vertexInput v)
				{
					...
					OUT.NdotV = 1.0 - max(dt, (1.0 - _NdotVAmount));
				...
				{
					/*
					float AlphaRegular = Main.a * Main.r * Mask.a * Mask.r * IN.col.a * _colorTint.a;
					float AlphaInverted = 1.0 - AlphaRegular;

					float Alpha = lerp(AlphaRegular, AlphaInverted, _MaskInv);

					float3 Color = saturate((Main.rgb * Mask.rgb * _colorTint.rgb * IN.col.rgb) + Alpha);
					Color = Color * _overbright;
					...
					*/
					...
					Alpha = 1.0 - (Alpha * IN.col.a * _alphaPow); //invert ALPHA due it is multiplicative blending...
					
					//calculate the color gradient based on the "alpha value"
					float tintMask = saturate(smoothstep(_Col_midPoint - _Col_thresh, _Col_midPoint + _Col_thresh, Alpha));
					float4 colTint = lerp(_colorTintB, _colorTintA, tintMask);
					float3 Color = lerp(colTint.rgb, Main.rgb, _Col_mix);
					Color = saturate(Color * IN.col.rgb + (Alpha + IN.NdotV));

					//output composite: RGB + A
					fixed4 Complete = float4(Color, 1.0);
